...
function save_inquiry_csv($data)
{
    ...
    if ($isNew) {
        fputcsv($fp, array('时间', 'IP', '姓名', '公司', '邮箱', '产品分类', '产品', '数量', '留言', '邮件是否发送成功'));
    }
    fputcsv($fp, $data);
    fclose($fp);
    return true;
...
function handle_inquiry()
{
    ...
    $body .= '邮箱: ' . $email . "\n";
    $body .= '产品分类: ' . ($category !== '' ? $category : '-') . "\n";
    $body .= '产品: ' . ($product !== '' ? $product : '-') . "\n";
    $body .= '数量: ' . ($qty !== '' ? $qty : '-') . "\n";
    $body .= '语言: ' . ($lang !== '' ? $lang : '-') . "\n";
    $body .= "-------------------------------\n";
    $body .= '留言内容:' . "\n" . $msg . "\n";
    $body .= "==============================\n";
    $body .= '来源页面: ' . (isset($_SERVER['HTTP_REFERER']) && $_SERVER['HTTP_REFERER'] !== '' ? $_SERVER['HTTP_REFERER'] : '-') . "\n";
    $body .= '提交时间: ' . date('Y-m-d H:i:s') . "\n";
    $body .= '客户IP: ' . (isset($_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR'] : '-') . "\n";
    ...
    save_inquiry_csv(array(
        ...
        $name, $company, $email, $category, $product, $qty, $msg,
        $sent ? '是' : '否'
    ));
